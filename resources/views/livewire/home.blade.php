<div class="flex flex-col min-h-screen" x-ref="mainContainer">
    <div class="bg-background-light text-[#111815] transition-colors duration-300 flex-1 flex flex-col">
        ...
        <main class="flex flex-col items-center flex-1">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div class="relative hero-image">
                        ...
                        <div class="relative w-full aspect-[4/3] bg-white rounded-xl shadow-2xl border border-[#dbe6e1] overflow-hidden p-4">
                            <div class="flex flex-col h-full bg-background-light rounded-lg p-4 gap-4">
            ...
            <section class="w-full bg-white py-12 stats-section" id="stats">
                <div class="max-w-[1200px] mx-auto px-4 md:px-10">
                    <div class="flex flex-wrap gap-6 justify-center">
                        <div class="flex min-w-[200px] flex-1 flex-col items-center gap-2 rounded-xl p-8 border border-[#dbe6e1] bg-background-light stats-card">
                            ...
                            <p class="text-[#111815] tracking-light text-4xl font-black leading-tight">
                                <span x-data="{ current: 0, target: 10000 }" x-intersect="$nextTick(() => { const duration = 2000; const start = Date.now(); const step = () => { const progress = Math.min((Date.now() - start) / duration, 1); current = Math.floor(progress * target); if (progress < 1) requestAnimationFrame(step); }; requestAnimationFrame(step); })" x-text="current + '+'"></span>
                            </p>
                        </div>
                        <div class="flex min-w-[200px] flex-1 flex-col items-center gap-2 rounded-xl p-8 border border-[#dbe6e1] bg-background-light stats-card">
                            <p class="text-gray-500 text-sm font-medium">Metas Concluídas</p>
                            <p class="text-[#111815] tracking-light text-4xl font-black leading-tight">
                                <span x-data="{ current: 0, target: 1.2 }" x-intersect="$nextTick(() => { const duration = 2000; const start = Date.now(); const step = () => { const progress = Math.min((Date.now() - start) / duration, 1); current = (progress * target).toFixed(1); if (progress < 1) requestAnimationFrame(step); }; requestAnimationFrame(step); })" x-text="current + 'M'"></span>
                            </p>
                        </div>
                        <div class="flex min-w-[200px] flex-1 flex-col items-center gap-2 rounded-xl p-8 border border-[#dbe6e1] bg-background-light stats-card">
                            <p class="text-gray-500 text-sm font-medium">Taxa de Sucesso</p>
                            <p class="text-[#111815] tracking-light text-4xl font-black leading-tight">
                                <span x-data="{ current: 0, target: 94 }" x-intersect="$nextTick(() => { const duration = 2000; const start = Date.now(); const step = () => { const progress = Math.min((Date.now() - start) / duration, 1); current = Math.floor(progress * target); if (progress < 1) requestAnimationFrame(step); }; requestAnimationFrame(step); })" x-text="current + '%'"></span>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <section class="w-full max-w-[1200px] px-4 py-24 cta-section">
                <div class="bg-background-dark text-white rounded-xl p-8 md:p-16 flex flex-col items-center text-center gap-8 relative overflow-hidden cta-content">
