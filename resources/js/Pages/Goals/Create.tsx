...
export default function CreateGoal({ goal }: CreateGoalProps) {
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        ...
        micro_tasks:
            goal?.micro_tasks?.map((t: any) => ({ title: t.title })) ||
            ([] as { title: string }[]),
        new_task_title: "",
    });
    ...
    const submit = (e: React.FormEvent) => {
        ...
        if (!data.is_streak_enabled && data.micro_tasks.length === 0) {
            setValErrors({
                ...valErrors,
                micro_tasks:
                    "A meta precisa ter pelo menos uma sub-tarefa ou o sistema de ofensivas ativado.",
            });
            return;
        }
    ...
    const addMicroTask = () => {
        ...
        if (title) {
            ...
            if (isDuplicate) {
                setValErrors({
                    ...valErrors,
                    new_task_title: "Esta tarefa já foi adicionada!",
                });
                return;
            }

            const updatedTasks = [...data.micro_tasks, { title: title }];
                ...
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="relative mb-12 w-full px-4 md:px-10">
                        ...
                        <div className="relative z-10 flex items-center justify-between w-full max-w-sm mx-auto">
                            ...
                            >
                                <div
                                    className={`size-10 rounded-full flex items-center justify-center font-bold border-2 transition-all group-hover:scale-110 ${step >= 1 ? "bg-primary text-background-dark border-primary shadow-lg shadow-primary/20" : "bg-white dark:bg-gray-800 border-gray-100 text-gray-400"}`}
                                >
                                    {step > 1 ? (
                                ...
                                <span
                                    className={`text-[10px] font-bold uppercase tracking-widest ${step >= 1 ? "text-primary" : "text-gray-400"}`}
                                >
                                    Objetivo
                                </span>
                            </div>
                            ...
                            >
                                <div
                                    className={`size-10 rounded-full flex items-center justify-center font-bold border-2 transition-all group-hover:scale-110 ${step >= 2 ? "bg-primary text-background-dark border-primary shadow-lg shadow-primary/20" : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-400"}`}
                                >
                                    2
                                </div>
                                <span
                                    className={`text-[10px] font-bold uppercase tracking-widest ${step >= 2 ? "text-[#111815] dark:text-white" : "text-gray-400"}`}
                                >
                                    Passos
                                </span>
                            </div>
                            ...
                            {step === 1 && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="col-span-1 md:col-span-2 space-y-2">
                                        <label className="block text-sm font-bold text-[#111815] dark:text-white">
                                    ...
                                    <div className="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {/* Streak Option */}
                                        <div
                                            className={`p-5 rounded-2xl border transition-all ${data.is_streak_enabled ? "bg-orange-50 dark:bg-orange-900/10 border-orange-100 dark:border-orange-800/30" : "bg-gray-50 dark:bg-gray-900/50 border-gray-100 dark:border-gray-700 opacity-60 hover:opacity-100"}`}
                                        >
                                            <div className="flex items-center justify-between w-full h-full">
                                                <div className="flex items-center gap-4 flex-1">
                                                    <div
                                                        className={`size-12 rounded-full flex items-center justify-center transition-colors ${data.is_streak_enabled ? "bg-orange-100 dark:bg-orange-900/40 text-orange-500" : "bg-gray-200 dark:bg-gray-800 text-gray-400"}`}
                                                    >
                                                        <span
                                                    ...
                                                    <div>
                                                        <label
                                                            className="flex items-center gap-2 text-sm font-extrabold text-[#111815] dark:text-white cursor-pointer"
                                                            htmlFor="enable-streak"
                                                        >
                                                            Habilitar Ofensiva?
                                                        </label>
                                                ...
                                                <div className="relative inline-block w-12 h-6 transition duration-200 ease-in-out shrink-0">
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            setData((prev) => ({
                                                                ...
                                                                deadline:
                                                                    checked
                                                                        ? ""
                                                                        : prev.deadline, // Reset deadline if enabling streak
                                                            }));
                                                            if (
                                                                checked &&
                                                                valErrors.micro_tasks
                                                            ) {
                                                                const newErrors =
                                                                    {
                                                                        ...valErrors,
                                                    ...
                                                    <label
                                                        htmlFor="enable-streak"
                                                        className={`toggle-label block overflow-hidden h-6 rounded-full cursor-pointer transition-colors duration-200 ${data.is_streak_enabled ? "bg-orange-500" : "bg-gray-300 dark:bg-gray-700"}`}
                                                    ></label>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {/* Deadline Option */}
                                        <div
                                            className={`p-5 rounded-2xl border transition-all ${data.deadline ? "bg-blue-50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-800/30" : "bg-gray-50 dark:bg-gray-900/50 border-gray-100 dark:border-gray-700 opacity-60 hover:opacity-100"}`}
                                        >
                                            <div className="flex flex-col gap-4 w-full">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-4">
                                                        <div
                                                            className={`size-12 rounded-full flex items-center justify-center transition-colors ${data.deadline ? "bg-blue-100 dark:bg-blue-900/40 text-blue-500" : "bg-gray-200 dark:bg-gray-800 text-gray-400"}`}
                                                        >
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            setData((prev) => ({
                                                                ...
                                                                is_streak_enabled:
                                                                    date
                                                                        ? false
                                                                        : prev.is_streak_enabled, // Disable streak if setting deadline
                                                            }));
                                                        }}
                                                    />
                                                </div>
                            ...
                            {step === 2 && (
                                <div className="space-y-6">
                                    <div className="flex justify-between items-center border-b border-gray-100 dark:border-gray-700 pb-4">
                                        <div>
                                            <h3 className="text-lg font-bold text-[#111815] dark:text-white">
                                                Checklist de Passos
                                            </h3>
                                            <p className="text-sm text-gray-500">
                                                Defina os marcos para atingir
                                                seu objetivo
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex flex-col md:flex-row gap-4">
                                            <div
                                                className={`flex-grow bg-white dark:bg-gray-900 rounded-xl px-4 h-12 flex items-center border ${valErrors.new_task_title ? "border-red-500" : "border-gray-200 dark:border-gray-600"} focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition-all`}
                                            >
                                                <input
                                            ...
                                            <button
                                                ...
                                                className="shrink-0 flex items-center justify-center gap-2 bg-primary text-background-dark h-12 px-6 rounded-xl text-sm font-bold shadow-lg shadow-primary/20 hover:scale-105 transition-all"
                                            >
                                    ...
                                    {data.micro_tasks.length > 0 && (
                                        <div className="space-y-4">
                                            {data.micro_tasks.map(
                                                (task: any, index: number) => (
                                                    <div
                                                        key={index}
                                                        className="flex items-center gap-4 p-4 bg-background-light dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-700 group hover:border-primary/50 transition-colors"
                                                    >
                                                        <div className="flex-1 space-y-1">
                                                            <label className="text-[10px] uppercase tracking-wider font-bold text-gray-400">
                                                                Título da Tarefa
                                                            </label>
                                                            <div className="text-sm font-bold text-[#111815] dark:text-white">
                                                                {task.title}
                                                            </div>
                                                        </div>
                                                        <button
                                    ...
                                    <div className="mt-8 p-6 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-800/30 flex gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
                                                Metas quebradas em tarefas
                                                menores de até 30 minutos são 3x
