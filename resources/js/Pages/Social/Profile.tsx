...
export default function SocialProfile({ auth, profileUser, posts, stats, xpInfo, streak }: ProfileProps) {
    ...
    const handleLike = (postId: number) => {
        router.post(route('social.post.like', postId), {}, { preserveScroll: true });
    };
        ...
        <AuthenticatedLayout>
                    ...
                    <div className="px-6 pb-6 relative">
                        <div className="flex flex-col md:flex-row items-end gap-6 -mt-16 md:-mt-20">
                            <div className="relative group shrink-0">
                                <div className="size-32 md:size-40 rounded-full border-[5px] border-white dark:border-gray-900 overflow-hidden bg-white shadow-xl relative group">
                                    <img 
                                        ...
                                        src={profileUser.avatar_url || `https://ui-avatars.com/api/?name=${profileUser.name}&size=200`} 
                                    />
                                    {isOwnProfile && (
                                        <div 
                                        ...
                                        >
                                            ...
                                            <input 
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (file) {
                                                        router.post(route('social.profile.avatar'), { image: file }, { forceFormData: true });
                                                    }
                                                }}
                                            />
                                        </div>
                            ...
                            <div className="flex-1 pb-2 space-y-3">
                                <div className="flex flex-wrap items-center gap-3">
                                    <h1 className="text-2xl md:text-3xl font-extrabold dark:text-white uppercase tracking-tighter">{profileUser.name}</h1>
                                    <p className="text-gray-500 dark:text-gray-400 font-medium">@{profileUser.nickname || profileUser.name.toLowerCase().replace(' ', '_')}</p>
                                    {isOwnProfile && (
                                        <Link href={route('profile.edit')} className="ml-2 text-xs font-bold text-gray-400 hover:text-primary transition-colors flex items-center gap-1">
                                ...
                                <div className="flex items-center gap-4 max-w-md">
                                    <div className="flex-1">
                                        <div className="flex justify-between items-end mb-1">
                                            <span className="text-[10px] font-black uppercase text-primary tracking-tighter">Nível {xpInfo.level}</span>
                                            <span className="text-[10px] font-bold text-gray-400">{xpInfo.current} / {xpInfo.next_level_threshold * xpInfo.level} XP</span>
                                        </div>
                                        <div className="h-1.5 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                            <div className="bg-gradient-to-r from-primary to-blue-400 h-full rounded-full" style={{ width: `${(xpInfo.current % 1000) / 10}%` }}></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            {!isOwnProfile && (
                                <div className="flex items-center gap-2 mb-2">
                                    <button 
                                        onClick={() => router.post(route(profileUser.is_following ? 'social.unfollow' : 'social.request.send', profileUser.id))}
                                    ...
                                    >
                                        {profileUser.is_following ? 'Parar de Seguir' : 'Adicionar amigo'}
                                    </button>
                                </div>
                            )}
                        </div>
                        <div className="mt-6 flex flex-wrap gap-6 border-t border-gray-50 dark:border-gray-800 pt-6">
                            <div className="flex gap-1 items-center">
                                <span className="font-bold dark:text-white">{stats.followers}</span>
                                <span className="text-gray-500 dark:text-gray-400 text-sm">Seguidores</span>
                            </div>
                            <div className="flex gap-1 items-center">
                                <span className="font-bold dark:text-white">{stats.following}</span>
                                <span className="text-gray-500 dark:text-gray-400 text-sm">Seguindo</span>
                            </div>
                            <div className="flex gap-1 items-center">
                                <span className="font-bold dark:text-white">{profileUser.goals_count || 0}</span>
                                <span className="text-gray-500 dark:text-gray-400 text-sm">Metas Batidas</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        {posts.length > 0 ? posts.map((post) => (
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                {post.type === 'goal_completed' && (
                                                    <span className="text-xs text-gray-400">concluiu uma meta</span>
                                                )}
                                            </div>
                                            <p className="text-xs text-gray-400 flex items-center gap-1"><span className="material-symbols-outlined text-[12px]">schedule</span> {formatDateTime(post.created_at)}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="px-4 pb-2">
                                    {post.type === 'goal_completed' && (
                                        <div className="bg-emerald-50 dark:bg-emerald-900/20 px-3 py-2 rounded-xl mb-3 flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wide">Meta Batida</span>
                                        </div>
                                    )}
                                    <p className="text-sm dark:text-gray-300 leading-relaxed whitespace-pre-wrap">{post.content}</p>
                                </div>
                                                ...
                                                <div key={comment.id} className="flex gap-2">
                                                    ...
                                                    <div className="flex-1 bg-gray-50 dark:bg-gray-800 p-2 rounded-xl">
                                                        <div className="flex justify-between items-center mb-0.5">
                                                            <Link href={route('social.profile', comment.user.nickname || comment.user.id)} className="text-[10px] font-black dark:text-white hover:underline">
                                                                {comment.user.name}
                                                            </Link>
                                                            <span className="text-[8px] text-gray-400">{formatDateTime(comment.created_at)}</span>
                                                        </div>
                            ...
                            <div className="text-center py-20 bg-white dark:bg-gray-900 border border-dashed border-gray-100 dark:border-gray-800 rounded-2xl">
                                <p className="text-gray-500 font-bold">Nenhum marco alcançado ainda.</p>
                            </div>
                        )}
                    </div>
                        ...
                        <section className="bg-gradient-to-br from-gray-900 to-black rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
                            <div className="relative z-10">
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Ritmo Atual</span>
                                </div>
                                <div className="flex items-baseline gap-1">
                                    <span className="text-4xl font-black">{streak}</span>
                                    <span className="text-xs font-bold text-gray-400 uppercase">{streak <= 1 ? 'Dia' : 'Dias'} de Ofensiva</span>
                                </div>
                                <div className="mt-4 h-1 w-full bg-white/10 rounded-full overflow-hidden">
                                    <div className="bg-orange-500 h-full transition-all duration-1000" style={{ width: `${Math.min((streak / 30) * 100, 100)}%` }}></div>
                                </div>
                                <p className="text-[10px] mt-2 text-gray-500 font-bold uppercase tracking-tight">Mantenha a chama acesa!</p>
                            </div>
            ...
            <Modal show={reportingPost !== null} onClose={() => setReportingPost(null)}>
                <form onSubmit={handleReportPost} className="p-6">
                    <h2 className="text-lg font-bold dark:text-white mb-4">Denunciar Postagem</h2>
                    <div className="space-y-4">
                        <div>
                            <InputLabel value="Motivo da Denúncia" children={undefined} />
                            <select 
                        ...
                        <div>
                            <InputLabel value="Detalhes (opcional)" children={undefined} />
                            <textarea 
                                className="w-full mt-1 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-primary focus:ring-primary rounded-xl shadow-sm"
                                rows={3}
                                value={reportDetails}
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        ...
                        <PrimaryButton disabled={!reportReason}>Enviar Denúncia</PrimaryButton>
                    </div>
                </form>
            </Modal>
            ...
            <Modal show={editingPost !== null} onClose={() => setEditingPost(null)}>
                <form onSubmit={handleUpdatePost} className="p-6">
                    <h2 className="text-lg font-bold dark:text-white mb-4">Editar Postagem</h2>
                    <div className="space-y-4">
                        <div>
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        <SecondaryButton onClick={() => setEditingPost(null)} disabled={false}>Cancelar</SecondaryButton>
                        <PrimaryButton disabled={!editContent}>Salvar Alterações</PrimaryButton>
                    </div>
                </form>
            </Modal>
            ...
            <Modal show={showDeleteModal !== null} onClose={() => setShowDeleteModal(null)}>
                <div className="p-6 text-center">
                    <h2 className="text-xl font-black dark:text-white mb-2 uppercase tracking-tighter">Confirmar Exclusão</h2>
                    <p className="text-gray-500 dark:text-gray-400 mb-6">Esta ação é permanente. Deseja mesmo excluir este post?</p>
