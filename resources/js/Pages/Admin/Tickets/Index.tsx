...
export default function TicketsIndex({ tickets, filters }: Props) {
    const handleSearch = (search?: string) => {
        router.get(route("admin.tickets.index"), search ? { search } : {}, {
            preserveState: true,
            preserveScroll: true,
        });
    };
        ...
        <AdminLayout>
            ...
            <div className="space-y-6">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            Atenda as solicitações de suporte e dúvidas dos
                            usuários.
                        </p>
                    </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-3xl border border-[#dbe6e1] dark:border-gray-700 shadow-sm overflow-hidden">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50/50 dark:bg-gray-900/50">
                                    <th className="px-8 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">
                                        Protocolo
                                    </th>
                                    <th className="px-8 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">
                                        Assunto
                            ...
                            <tbody className="divide-y divide-[#dbe6e1] dark:divide-gray-700">
                                {tickets.data.map((ticket) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-8 py-5">
                                            <span
                                                className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${ticket.status_color}`}
                                            >
                                                {ticket.status_label}
                                            </span>
                                        </td>
                    ...
                    {/* Mobile Card View */}
                    <div className="lg:hidden divide-y divide-[#dbe6e1] dark:divide-gray-700">
                        {tickets.data.map((ticket) => (
                            <div key={ticket.id} className="p-4 space-y-3">
                                ...
                                <div className="flex items-center justify-between pt-2">
                                    <div className="text-xs text-gray-400 break-all pr-4">
                                        {ticket.email}
                                    </div>
                                    <Link
                                        href={route(
                                        ...
                                        className="shrink-0 inline-flex items-center justify-center h-9 px-4 rounded-xl bg-primary text-gray-900 shadow-sm font-bold text-xs"
                                    >
                                        Ver Detalhes
                                    </Link>
                                </div>
